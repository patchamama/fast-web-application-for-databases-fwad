 	...
 	$aggfield = false,$sumlabel='Sum ',$aggfn ='SUM', $showcount = true)
 {
		...
		foreach ($colarr as $v) {
			...
			if (!$hidecnt) {
				$sel .= $iif ?
					...
					:
					"\n\t$aggfn(CASE WHEN $colfield=$vq $vaggNull THEN 1 ELSE 0 END) AS \"$v\", ";
			...
			if ($aggfield) {
				...
				$sel .= $iif ?
					...
					:
					"\n\t$aggfn(CASE WHEN $colfield=$vq $vaggNull THEN $aggfield ELSE 0 END) AS \"$label\", ";
			}
		}
	}
	...
	while (@eregi ('(.*),(.*)', $temp, $match1))
		...
		if (@eregi ('(.*) as (.*)', $match1[2], $match2))
			{
			$groupfields = (strlen($groupfields)>0)? $match2[1].', '.$groupfields: $match2[1];
			}
		else	{
			$groupfields = (strlen($groupfields)>0)? $match1[2].', '.$groupfields: $match1[2];
			}
		}
	if (@eregi ('(.*) as (.*)', $temp, $match2))
		...
		$groupfields = (strlen($groupfields)>0)? $temp.', '.$groupfields: $temp;
		}
