...
$button_strings['Prev'] = "<img src='images/action_back.gif' />";
$button_strings['Next'] = "<img src='images/action_forward.gif' />";
$Confs["HelpIco"] = "<img src='images/icon_info.gif' />";
$Confs["EditIco"] = "<img src='images/page_edit.gif' />";
$Confs["UserIco"] = "<img src='images/icon_user.gif' />";
$Confs["SearchIco"] = "<img src='images/page_find.gif' />";
...
<head>
  <title>CollMan: Manejo de Colecciones Biológicas</title>
	...
	<meta name="Description" content="Gesti?n de informaci?n biol?gica, ecol?gica, etnobot?nica, taxon?mica, literatura, Sistemas de informacion geogr?fica (SIG)." />
					...
					if (isset($s_fields_value[$vTable]))
						...
						if ($vcontinue)
							...
							if ($rec === FALSE) {
								...
								echo "<br />Error ejecutando SQL: $vSql <br />";
...
//-----------------------------------------------------------------------------------------
	...
	for ($i = 0; $i < count($s_xml_conf['searchs']); $i++)
		...
		if (get_value($s_xml_conf['searchs'][$i],'tagname')=='sql')
			...
			if (@preg_match('|.*select .+ from (.+)( where .*)?|im', $vSqlSearch, $match))
				...
				if (!empty($HTTP_PARAM_VARS['query_run']))
					{
									...
									if ($s_xml_conf['elements'][$vnelem]['q_conditions'][$vinModName][$vcond]['level']==$level)
										...
										if ( ($vtype=='textbox_list') )
											...
											elseif (@preg_match('|.*select .+ from (.+) (where(.*))?|Uim', $s_xml_conf['elements'][$vnelem]['sql'], $match))
												...
												$vValueBefore ='('.ereg_replace('#1', $vtid, $syntax['select']);
												...
												$vValueAfter = ')';
												if ( (!array_key_exists($vvalue, $unaryCOP)) and ($vcomparison == $binaryCOP['like']) and (strpos($vvalue,$syntax['wildcard'])==0))
													{
										...
										if ($vSqlGo)
											...
											if ( (!array_key_exists($vvalue, $unaryCOP)) and ($vcomparison == $binaryCOP['like']) and (strpos($vvalue,$syntax['wildcard'])==0) and (empty($vValueBefore)) and (empty($vValueAfter)) )
												{
												$vvalue = $syntax['wildcard'].$vvalue.$syntax['wildcard'];
												}
					...
					if ( isset($HTTP_PARAM_VARS['giveme_sql']) )
					...
					else	{ //maybe there is a SQL query in other module
						...
						for ($i=0; $i < count($s_xml_conf['QueryModRelations']); $i++)
							...
							if ( !empty($vRelModule) )
								...
								if ( ($HTTP_PARAM_VARS["query_runClicked"] == "go") &&
			...
			if (($HTTP_PARAM_VARS['query_dest']=='form') and
				(!empty($HTTP_PARAM_VARS['query_run'])) )
				{
					...
					while ( (!$rec->EOF) )
						...
						if ($vposi<=$vtini+$vQueryCountRun)
							...
							if ((trim($vSelected)==trim($vvvalue))  and (strlen($vSelected)>0))
								...
								$vtSearch .=  '<option value="'.$vvvalue.'" selected>'.$rec->fields[$vDesc]."</option>\n";
								...
								$vtSearch .=  '<option value="'.$vvvalue.'" >'.$rec->fields[$vDesc]."</option>\n";
								}
							}
						else	{
	...
	if ($vShowForm)
		...
		if (($HTTP_PARAM_VARS['query_dest']=='form') and
			...
			$vv = ereg_replace('&&','&', $vv);
			$vv = ereg_replace('&&','&', $vv);
			$vLink  .=  '    <a href="javascript:Redirect('."'".'run.php?'.$vv.'&Search=Go&SelectSqlParam='.$HTTP_PARAM_VARS['SelectSqlParam'].'#markGo'."'".')">'.$button_strings['LeaveQuery'].'</a>';
			}
			...
			if ((count($s_xml_conf['elements'])>0) and ($vShowForm))
				{
					...
					if ($vdata!='all') //if was'nt selected data the fields will be grouping, dont show the panel...
						{
						$panel  .=  "\n".'<table width="'.$vFormPercent.'%" class="Panel" cellpadding="2" cellspacing="0">'
								."    <tr>\n"
								.' 		<td width="20" class="Panel">'
								.'&nbsp;&nbsp;'
								." 		</td>\n";
						}
...
*/
								...
								if ($vsectionbr)
									{
									$panel  .='      <td height="20" width="100%" class="Panel">'
										.'&nbsp;'
										.'      </td>'
										.'   </tr>'
										."</table>\n"
								...
								if (($vfirst) or ($vdata=='panel_'.($vsection_cant))) 
									...
									$panel .='      <td nowrap class="PanelSelected">'
										.'          <a href="javascript:NewPanel(\'panel_'.$vsection_cant.'\');">'
										.'&nbsp;&nbsp;'.$vsection.'&nbsp;&nbsp;'
										.'         </a>'
										."      </td>\n";
									}
									...
									$panel .='      <td nowrap class="PanelUnSelected">'
										.'         <a href="javascript:NewPanel(\'panel_'.$vsection_cant.'\');">'
										.'&nbsp;&nbsp;'.$vsection.'&nbsp;&nbsp;'
										.'         </a>'
										."     	</td>\n";
							...
							elseif (get_value($s_xml_conf['elements'][$vnelem],'tagname')=='element')
								{ //ini of field tag part
									...
									$vDateFrmtEnter  = (IsEmpty($vDateFrmtEnter))? 'd/m/y': $vDateFrmtEnter;
									...
									$vDateFrmtSave  = (IsEmpty($vDateFrmtSave))? 'y/m/d': $vDateFrmtSave;
...
//print "<br />otro".$vName;
	...
	//===========================================================================================================================================================
										...
										elseif (($vType=='textbox_list') and ($vSqlOk))
											...
											if (strlen($vSelected)>0)
												...
												if ($rec === FALSE) {
													$db_error .= $dbhandle->ErrorMsg();
													echo "<br />Error ejecutando SQL: $vSql1 <br />";
													echo $db_error."<br /><pre>";
													print_r($s_xml_conf['elements'][$vnelem]);
													echo "</pre>";
													require('./inc/script_end.inc.php');
										...
										elseif (($vType=='listbox_combobox') and ($vSqlOk))
											...
											if ($vmultiRecords)
												...
												if ($checkSearch) //the values of the form are the result of a search at this moment......
													...
													if ($recTemp === FALSE) {
														$db_error .= $dbhandle->ErrorMsg();
														echo "<br />Error ejecutando SQL: $vSqlSearch <br />";
														echo $db_error."<br /><pre>";
														print_r($s_xml_conf['elements'][$vnelem]);
														echo "</pre>";
														require('./inc/script_end.inc.php');
	...
	//===============================================================================================================================================================================
										...
										if (strlen($vHelp)>0)
											{
											$vHelp = $pathMod."help/".$s_xml_conf['lang']."/".$vHelp;

											if ($vHelpOpenNewWindow)
												{
												$vHTML .=  '<a href="'.urlencode($vHelp.'?keepThis=true&TB_iframe=true&height=250&width=700').'" title="'.$message_strings['Help'].'" class="thickbox">'.$Confs["HelpIco"].'</a>';
												}
											else	{
					...
					if (($vdata!='all')  )
					{
						...
						$vHTML  .=  '<div '. (($vdata!='panel_goquery')? 'style="display: none;"': "") .' id="panel_goquery">';  //inicio del div del panel...
							...
							$vHTML .=  '<tr><td>';
								...
								$panel .=	'      <td nowrap class="PanelQueryUnselected">'
										.'         <a href="javascript:NewPanel(\'panel_goquery\');">'
								...
								//Section of queries parameters...
									...
									$vHTML .=  '<table width="'.$vFormPercent.'%" border="1" cellpadding="4" cellspacing="0" class="TableFieldValue">';
									$vHTML  .=  '<tr><td align="right" class="TableField">';
										{
										...
										$vHTML .=  "\n".'</td><td width="90%" class="TableFieldValue">';
										//--------------------------------------------------------------
...
function Today(theField,FieldSave, formatEnter, formatSave, IsEmptyOK, s, vWithAccept)
{
	...
	CheckDate(theField, FieldSave, 'd-m-y', formatSave, IsEmptyOK, s, vWithAccept);
}
			...
			if (isset($s_xml_conf['configuration']))
					...
					for ($vaa=0; $vaa<count($s_xml_conf['configuration']); $vaa++)
						...
						if (get_value($s_xml_conf['configuration'][$vaa],'tagname')=='info')
							{
							echo "\n".'<table width="'.$vFormPercent.'%" border="0" cellpadding="4" cellspacing="0" class="TableFieldValue">';
							echo "\n".'<tr><td class="Panel">';
